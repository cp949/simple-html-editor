...
3. 기존 공개 API, HTML round-trip, dist 4파일 계약과 라이선스 allowlist를 유지한다.
...
공개 `.d.ts`에는 React peer 이외에 Tiptap/ProseMirror 구현 타입을 노출하지 않는다.
...
- dist 정확히 4파일과 public declaration boundary 유지
